...
module.exports = async member => {
    ...
    if (kurulus > 604800000) {
        ...
        member.guild.channels.cache.get(config.channels.welcomechannel).send(`:tada: Hey! Sunucumuza hoş geldin ${member}!
...
Seninle birlikte **${member.guild.memberCount}** kişiye ulaştık.`);
        ...
        member.setNickname(config.registration.autonickname);
        member.guild.channels.cache.get(config.channels.welcomechannel).send({ embeds: [embed.setDescription(`${member}, kullanıcısı sunucuya katıldı hesabı **${zaman2}** önce açıldığı için şüpheli.`)]})
    }
}
